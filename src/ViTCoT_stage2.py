...
# API key pool (keeping original key sequence)
API_KEYS = []
...
class GeminiVideoAnalyzer:
    ...
    def _init_client(self):
        """Retain original API key initialization logic"""
        # genai.configure(api_key=API_KEYS[self.api_index])
        self.client = genai.Client(api_key=API_KEYS[self.api_index])

    def _rotate_key(self):
        """Maintain existing key rotation mechanism"""
        self.api_index = (self.api_index + 1) % len(API_KEYS)
        self._init_client()
        logging.warning(f"API key rotated to index {self.api_index}")

    def generate_response(self, video_path, video_path_key_video, final_query1, final_query2, model_output_1, max_retries=10):
        """Retain original inference logic for Gemini implementation"""
        for _ in range(max_retries):
            try:
                video_file = upload_video(video_path, self.client)
                video_file_key_video = upload_video(video_path_key_video, self.client)
                response = generate_content(video_file, video_file_key_video, final_query1, final_query2, model_output_1, self.client)
                return response
...
def generate_content(video_file, video_file_key_video, final_query1, final_query2, model_output_1, client):
    contents = [
        video_file,
        {
            "role": "user",
    ...
    # print(f"contents: {[video_file, prompt]}")
...
def build_prompt(item, with_evidence):
    """Generate prompt based on new logic"""
    ...
    question = item["question"]
    task = item['task']
    model_output_1 = item['model_output_1']
    ...
    if with_evidence:
        ...
        final_query1 = f"Please finish the {task} task. Question: {question} You have the following options: {options_prompt}. \nLet's think step by step!"
        final_query2 = f"Let's think step by step! \nNote, you must choose one of the four options and provide your final answer in this format: `Answer: A or B or C or D`."

    return final_query1, final_query2, model_output_1

def main():
    """Retain original main program structure"""
    parser = argparse.ArgumentParser(description='ViTCoT Video Reasoning')
    parser.add_argument('-sn',  '--save_path', default='vitcot_stage2.jsonl', help='Result save path')
    parser.add_argument('-we',  '--with_evidence', action='store_true', help='Evidence mode')
    parser.add_argument('-jp',  '--json_path', default='vitcot_stage1.jsonl',  help='Input file')
    ...
    # Retain original data loading method
    ...
    # Set total to the size of data being processed
    total = end_idx - start_idx
    
    # Retain progress bar display
    for i, item in enumerate(tqdm(dataset[start_idx:end_idx], desc='Video Analysis Progress')):
        try:
            video_path =  item["video_path"]
            video_path_key_video = item["key_video_path"]
            final_query1, final_query2, model_output_1 = build_prompt(item, args.with_evidence)

            # Execute inference
            response = analyzer.generate_response(video_path, video_path_key_video, final_query1, final_query2, model_output_1)
            print(response)
            ...
            output_item = {
                ...
                "task": item.get("task",  ""),
                "final_query": final_query1 + final_query2,
                "model_output_1": model_output_1,
